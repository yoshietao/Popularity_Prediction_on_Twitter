	...
	if Path('./data/'+filename+'Q1_5x.npy').exists():
		...
		with open(filename+'.txt') as data:
			for line in data:
				...
				if index1 in x:
					x[index1][0] += 1
					x[index1][1] += int(line['metrics']['citations']['total'])
					x[index1][2] += float(line['author']['followers'])
					x[index1][3] += int(line['metrics']['momentum'])
					if int(line['tweet']['user']['friends_count']) > x[index1][4]:
						x[index1][4] = int(line['tweet']['user']['friends_count'])
				else:
					x[index1] = [0,0,0,0,0]
